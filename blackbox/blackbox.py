...
class DryRunTransactionResult:
    ...
    * `noError`
    ...
    def report(self, args: Iterable[Union[str, int]], msg: str, row: int = 0) -> str:
        bbr = self.black_box_results
        return f"""===============
    <<<<<<<<<<<{msg}>>>>>>>>>>>
    ===============
    ...
    def extract_all(cls, txn: dict, is_app: bool) -> dict:
        ...
        result["bbr"] = BlackboxResults.scrape(result["trace"], result["lines"])
        ...
        * functions which take _two_ variables. These are useful when your assertion is more
        ...
        if assertions:
            ...
            for key in assertions:
                assert isinstance(key, DRProp) and SequenceAssertion.mode_has_assertion(
                    mode, key
